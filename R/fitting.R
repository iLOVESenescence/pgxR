...
#'
#' @param data Aggregated data frame from [combine_reps()].
...
#' fit  <- fit_cl(agg, "SKMM-1")
#' }
fit_cl <- function(data, cell_line_name, model = drc::LL.4()) {
  validate_columns(data, c("cell_line", "dose", "mean_response"))
...
#' [fit_cl()] for each, returning a named list of `drc` model objects.
#'
#' @param data Aggregated data frame from [combine_reps()].
#' @param cell_lines Character vector of cell line names. Typically
#'   `unique(agg$cell_line)`.
#' @param model A `drc` model function passed to [fit_cl()].
#'   Default `drc::LL.4()`.
#'
...
#' }
fit_all <- function(data, cell_lines, model = drc::LL.4()) {
  if (length(cell_lines) == 0) {
    stop("'cell_lines' must contain at least one cell line name.", call. = FALSE)
  }
  
  fits <- purrr::map(cell_lines, ~fit_cl(data, .x, model = model))
